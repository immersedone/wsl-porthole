...
async function addPort(c: ContainerSummary, p: ContainerSummary["ports"][0]) {
  ...
  const direction = engine.value === "wsl" ? "WinToWsl" : "WslToWin";
  const connectAddr = engine.value === "wsl" ? "${WSL_IP}" : "${HOST_GW}";
  try {
...
<template>
  <div>
    <div class="flex items-center justify-between mb-4">
      <div>
        ...
        <p class="text-sm mt-1" :style="{ color: 'var(--text-secondary)' }">
          Discover running Docker containers and forward their ports.
          <template v-if="engine === 'wsl'">Showing containers from WSL Docker engine (unix socket).</template>
          <template v-else>Showing containers from Windows Docker engine (named pipe).</template>
        </p>
      </div>
      <div class="flex items-center gap-2">
        <select v-model="engine" class="text-xs px-2 py-1 rounded"
          :style="{ background: 'var(--bg-secondary)', color: 'var(--text-primary)', border: '1px solid var(--border)' }"
          title="Switch between WSL and Windows Docker engines">
          <option value="wsl">WSL Engine</option>
          <option value="windows">Windows Engine</option>
        </select>
    ...
    <div v-for="(ctrs, project) in grouped" :key="project" class="mb-6">
      <h3 class="text-xs font-semibold uppercase tracking-wider mb-2" :style="{ color: 'var(--text-secondary)' }">
        {{ project === "(standalone)" ? "Standalone Containers" : `Compose Project: ${project}` }}
      </h3>
      <div class="space-y-2">
        <div v-for="c in ctrs" :key="c.id" class="rounded-lg p-3" :style="{ background: 'var(--bg-secondary)', border: '1px solid var(--border)' }">
          <div class="flex items-center gap-3 mb-2">
            ...
            <span class="text-xs font-mono" :style="{ color: 'var(--text-secondary)' }" :title="c.image">{{ c.image }}</span>
            <span class="text-xs px-1.5 py-0.5 rounded" :style="{ background: c.status.startsWith('Up') ? 'rgba(63,185,80,0.15)' : 'rgba(248,81,73,0.15)', color: c.status.startsWith('Up') ? 'var(--status-ok)' : 'var(--status-err)' }">{{ c.status }}</span>
          </div>
          <div v-if="c.ports.length" class="flex flex-wrap gap-2">
            <div v-for="(p, i) in c.ports" :key="i" class="flex items-center gap-1.5 text-xs px-2 py-1 rounded" :style="{ background: 'var(--bg-tertiary)' }">
              ...
              <span v-if="hasRule(p.host_port)" class="flex items-center gap-0.5 text-[10px] px-1 rounded" :style="{ background: 'var(--status-ok)', color: '#000' }">
                <CheckCircle :size="9" /> forwarded
              </span>
              <button v-else @click="addPort(c, p)" class="flex items-center gap-0.5 text-[10px] px-1 rounded hover:opacity-80"
                :style="{ background: 'var(--accent-dim)', color: 'var(--text-primary)' }"
                :title="`Create a port forwarding rule for port ${p.host_port}`">
                <Plus :size="10" /> forward
              </button>
            </div>
          </div>
    ...
    <div v-if="!containers.length && !loading" class="text-center py-12" :style="{ color: 'var(--text-secondary)' }">
      No running containers found on the {{ engine === 'wsl' ? 'WSL' : 'Windows' }} Docker engine.
    </div>
  </div>
</template>
